...
void selectionSort( vector<Comparable> & a )
{
	...
	int end = a.size();	
	if (end != 0){
		for (curr; curr <= end-1; curr++){
			int minElemIdx = curr;
			int temp = curr;
			for (temp; temp <= end-1; temp++){
				if (a.at(temp) < a.at(minElemIdx)){
					minElemIdx = temp;
				}
			}
			Comparable tempEl = a.at(curr);
			a.at(curr) = a.at(minElemIdx);
			a.at(minElemIdx) = tempEl;
		}
	}
}
...
bool isSorted( vector<Comparable> & a )
{
	...
	if (a.empty()){
		...
		Comparable current = a.at(0);
		Comparable next = a.at(1);
		while (checking){
...
void hybridSort( vector<Comparable> & a )
{
    ...
    if (vecSize < 16){
		...
		int qtrSize = vecSize / 4;
		quicksort(a, 0, qtrSize);
		quicksort(a, qtrSize + 1, 2 * qtrSize);
		quicksort(a, 2 * qtrSize + 1, 3 * qtrSize);
		quicksort(a, 3 * qtrSize + 1, vecSize-1);
		merge(a, a, 0, qtrSize + 1, 2 * qtrSize);
		merge(a, a, 2 * qtrSize + 1, 3 * qtrSize + 1, vecSize - 1);
		merge(a, a, 0, 2 * qtrSize + 1, vecSize-1);
	}
}
